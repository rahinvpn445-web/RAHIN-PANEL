...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* ===== RAHIN Panel Layout & Sidebar (v1.1.0) ===== */
        :root {
            --rahin-sidebar-collapsed: 76px;
            --rahin-sidebar-expanded: 260px;
        }
        #app-sidebar {
            position: fixed;
            top: 0;
            left: 0;
            bottom: 0;
            width: var(--rahin-sidebar-collapsed);
            z-index: 40;
            background: linear-gradient(180deg, #0a0f1c 0%, #0b1324 100%);
        ...
        #app-sidebar.expanded {
            width: var(--rahin-sidebar-expanded);
        }
        #content-wrap {
            margin-left: var(--rahin-sidebar-collapsed);
            transition: margin-left .3s cubic-bezier(.4,0,.2,1);
            min-height: 100vh;
        }
        #content-wrap.sidebar-expanded {
            margin-left: var(--rahin-sidebar-expanded);
        }
        .rahin-sidebar-label {
            opacity: 0;
            max-width: 0;
        ...
        #app-sidebar.expanded .rahin-sidebar-label {
            opacity: 1;
            max-width: 160px;
            transition: opacity .25s ease .08s, max-width .3s cubic-bezier(.4,0,.2,1);
        }
        .rahin-nav-item {
            position: relative;
            display: flex;
        ...
        .rahin-nav-item:hover {
            color: #cbd5e1;
            background: rgba(148, 163, 184, 0.06);
        }
        .rahin-nav-item.active {
            color: #22d3ee;
            background: rgba(34, 211, 238, 0.08);
        }
        .rahin-nav-item.active::before {
            content: '';
            position: absolute;
            right: 0;
            top: 8px;
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen transition-colors duration-200">
    ...
    <aside id="app-sidebar">
        <div class="flex items-center gap-3 px-5 py-5 flex-shrink-0" style="min-height:72px;">
            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-400 to-indigo-500 flex items-center justify-center flex-shrink-0 shadow-lg shadow-cyan-500/20">
                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
            </div>
            <span id="sidebar-logo-text" class="rahin-sidebar-label text-white font-black text-lg" dir="ltr">RAHIN <span class="text-cyan-400 font-medium">Panel</span></span>
        </div>
        <button onclick="toggleSidebar()" title="باز/بسته کردن منو" class="flex items-center gap-3 px-5 py-3 mx-3 mb-2 rounded-xl text-slate-400 hover:text-cyan-300 hover:bg-white/5 transition-colors flex-shrink-0">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"></path></svg>
            <span class="rahin-sidebar-label text-sm font-medium">بستن منو</span>
        </button>
        <nav class="flex-1 flex flex-col gap-1 mt-2 overflow-y-auto">
            <div id="nav-dashboard" onclick="switchView('dashboard')" class="rahin-nav-item active">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <span class="rahin-sidebar-label text-sm font-semibold">Dashboard</span>
            </div>
            <div id="nav-users" onclick="switchView('users')" class="rahin-nav-item">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                <span class="rahin-sidebar-label text-sm font-semibold whitespace-nowrap">User Management</span>
            </div>
            <div id="nav-settings" onclick="toggleSettingsModal(true)" class="rahin-nav-item">
