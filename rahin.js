...
const Router = {
	...
	async handleSubscription(url, env) {
		...
		try {
			...
			let configName = "RAHIN_VPN";
			try {
				const configNameRow = await env.DB.prepare("SELECT value FROM settings WHERE key = 'config_name'").first();
				if (configNameRow && configNameRow.value) configName = configNameRow.value;
			} catch (e) {}
			return await SubscriptionService.generateText(user, host, configName);
		} catch (err) {
	...
	async handleUserStatus(url, env) {
		...
		try {
			...
			const html = HTML_TEMPLATES.status.replace("/* {{USER_DATA_PLACEHOLDER}} */", `window.statusUser = ${userJson}; window.currentConfigName = ${JSON.stringify(configName)};`);
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/rahinvpn445-web/RAHIN-PANEL/refs/heads/main/rahin.js?t=" + Date.now() + Math.random(), {
					headers: {
		...
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			...
			try {
				const githubRes = await fetch("https://raw.githubusercontent.com/rahinvpn445-web/RAHIN-PANEL/refs/heads/main/rahin.js?t=" + Date.now(), {
					headers: {
						"Cache-Control": "no-cache, no-store, must-revalidate",
						Pragma: "no-cache",
						Expires: "0",
		...
		if (url.pathname === "/api/proxy-ip") {
			if (request.method === "POST") {
				const { proxy_ip, iata, config_name } = await request.json();
				...
				if (config_name !== undefined) {
					const trimmedConfigName = String(config_name).trim();
					await env.DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('config_name', ?)").bind(trimmedConfigName || "RAHIN_VPN").run();
				}
				return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
			}
			if (request.method === "GET") {
				...
				const rowConfigName = await env.DB.prepare("SELECT value FROM settings WHERE key = 'config_name'").first();
				return new Response(
					JSON.stringify({
						proxy_ip: rowIp ? rowIp.value : "proxyip.cmliussss.net",
						iata: rowIata ? rowIata.value : "",
						config_name: rowConfigName && rowConfigName.value ? rowConfigName.value : "RAHIN_VPN",
					}),
...
const SubscriptionService = {
	async generateText(user, host, configName) {
		const cfgName = (configName && String(configName).trim()) || "RAHIN_VPN";
		let ips = [host];
		if (user.ips) {
		...
		const links = [];
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2FIn_Panel_Rayeghan_Ast_Va_Gheyre_Ghabele_Foroosh#" + encodeURIComponent("@Rahin_vpn1"));
		...
		const infoRemark = cfgName + " 📊 remaining | \u200E" + remVol + " | \u200E" + remTime + " | \u200E" + remReq;
		...
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				...
				const remark = cfgName + " | " + user.username + " | " + ip + " | " + portStr;
    ...
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-6">
        ...
        <div id="recovery-section" class="hidden">
            ...
            <div class="mb-5 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-xl text-xs leading-relaxed text-orange-800 dark:text-orange-300">
                ...
                <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=RAHIN-Panel-Token" target="_blank" class="mt-3 w-full flex items-center justify-center gap-2 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-bold transition shadow-md shadow-orange-500/20">
...
<div id="free-panel-warning-modal" class="fixed inset-0 z-[85] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-rose-500/50 rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-2">خوش‌آمدید</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            👋 به <span class="text-rose-500 font-bold">RAHIN Panel</span> خوش آمدید.<br>
            از اینکه از پنل ما استفاده می‌کنید سپاسگزاریم.<br><br>
            کانال رسمی:<br>
            <span class="font-bold" dir="ltr">@Rahin_vpn1</span>
        </p>
    ...
    <div id="settings-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh]">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain">
                ...
                <div>
                    <label class="block text-sm font-medium mb-1.5 text-gray-700 dark:text-zinc-300">Config Name</label>
                    <input type="text" id="config-name-input" placeholder="RAHIN_VPN" value="RAHIN_VPN" dir="ltr" class="w-full px-3 py-2.5 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-mono text-gray-700 dark:text-zinc-200">
                    <p class="text-[11px] text-gray-500 dark:text-zinc-400 mt-1">این نام برای همه کانفیگ‌ها و Subscriptionهای جدید (V2RayNG، Nekobox، Hiddify، Streisand و ...) استفاده می‌شود.</p>
                </div>
...
function getVlessLink(username) {
            ...
            if (!user) return '';
            const cfgName = window.currentConfigName || 'RAHIN_VPN';
            ...
            const userFrag = (user.frag_len && user.frag_int) ? '&fragment=' + user.frag_len + ',' + user.frag_int : '';
            const links = [];
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FIn_Panel_Rayeghan_Ast_Va_Gheyre_Ghabele_Foroosh#' + encodeURIComponent('@Rahin_vpn1'));
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
...
async function loadLocations() {
    ...
    try {
        ...
        if (statusRes.ok) {
            ...
            window.currentConfigName = statusData.config_name || 'RAHIN_VPN';
            const configNameInput = document.getElementById('config-name-input');
            if (configNameInput) configNameInput.value = window.currentConfigName;
        }
...
async function saveSettings() {
    ...
    try {
        ...
        const configNameInput = document.getElementById('config-name-input');
        const configNameVal = (configNameInput && configNameInput.value.trim()) || 'RAHIN_VPN';
        const response = await fetch('/api/proxy-ip', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ proxy_ip: resolvedIp, iata: iata ? iata.toUpperCase() : '', config_name: configNameVal })
        });
        if (response.ok) {
            window.currentConfigName = configNameVal;
        ...
        function exportUsersBackup() {
            ...
            downloadAnchor.setAttribute("download", "rahin_users_backup_" + dateStr + ".json");
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/rahinvpn445-web/RAHIN-PANEL/refs/heads/main/rahin.js?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/rahinvpn445-web/RAHIN-PANEL/refs/heads/main/ips.txt');
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پنل RAHIN - وضعیت اشتراک</h1>
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex items-center gap-4 justify-center">
        ...
        <a href="https://t.me/Rahin_vpn1" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 group">
        ...
        function getVlessLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    var remark = ips.length > 1 ? (cfgName + ' | ' + u.username + '-' + (ipIndex + 1) + '-' + portStr) : (cfgName + ' | ' + u.username + '-' + portStr);
