...
// This is the single source of truth for where RAHIN Panel pulls its own
// update source from. Only this block, the server-side update/restart
// routes, and the client-side "checkForUpdates" logic were touched to
// rebuild the Update System.
const RAHIN_REPO_URL = "https://github.com/rahinvpn445-web/RAHIN-PANEL";
...
function buildRahinSourceUrl() {
	// Unique query string on every call so neither GitHub's CDN (Fastly)
	// nor Cloudflare's edge/subrequest cache can ever serve a stale copy.
	return RAHIN_RAW_SOURCE_BASE + "?cb=" + Date.now() + "-" + Math.random().toString(36).slice(2);
}
function rahinNoCacheFetchInit(extraHeaders = {}) {
	return {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const errorMsg = err.message + " | در صورت عدم موفقیت، از طریق لینک زیر آپدیت کنید: https://rahin-panel.example.workers.dev/";
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 text-center mb-2" dir="ltr">Welcome to RAHIN Panel.<br>Manage your VPN configurations safely and easily.</p>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl p-6">
        <div id="login-section">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 text-center mb-6" dir="ltr">Welcome to RAHIN Panel.<br>Manage your VPN configurations safely and easily.</p>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="https://rahin-panel.example.workers.dev/" target="_blank" class="w-full py-3.5 bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-orange-600 dark:text-orange-500 border border-orange-300 dark:border-orange-500 font-bold rounded-xl text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div id="token-modal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        <div id="token-modal-card" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-6 transform transition-all scale-95 opacity-0 duration-200">
            ...
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=RAHIN-Deployer-Token" target="_blank" class="flex items-center justify-center gap-2 w-full py-3 bg-[#d94800] hover:bg-[#e35802] text-white font-bold rounded-xl text-sm transition duration-300 mb-4 shadow-md shadow-orange-500/20">
...
<div id="custom-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    ...
    <script>
        // One-time migration of localStorage keys from a prior "Zeus" installation to RAHIN.
        // Copies any legacy zeus_*-prefixed key to its rahin_ equivalent (without overwriting
        // an existing rahin_ value) and removes the old key. Safe to run on every load.
        (function migrateLegacyZeusStorageKeys() {
            try {
                const keysToMigrate = [];
                for (let i = 0; i < localStorage.length; i++) {
                    const key = localStorage.key(i);
                    if (key && key.indexOf('zeus_') === 0) keysToMigrate.push(key);
                }
                keysToMigrate.forEach((oldKey) => {
                    const newKey = 'rahin_' + oldKey.slice('zeus_'.length);
        ...
        function renderUsersUI(data) {
            try {
                ...
                if (cfRequests >= 90000) {
                    ...
                    const today = new Date().toISOString().split('T')[0];
                    if (localStorage.getItem('rahin_usage_warned_date') !== today) {
                        openUsageWarning();
                    }
                } else {
                    if (reqCard) {
...
function closePathWarning() {
    ...
    card.classList.remove('opacity-100', 'scale-100');
    card.classList.add('opacity-0', 'scale-95');
    localStorage.setItem('rahin_path_warned_' + CURRENT_VERSION, 'true');
}
function closeUsageWarning() {
    ...
    card.classList.add('opacity-0', 'scale-95');
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem('rahin_usage_warned_date', today);
}
    ...
    toggleModal(true);
        ...
        async function exportUsersBackup() {
            ...
            try {
                ...
                downloadAnchor.setAttribute("download", "rahin_full_backup_" + dateStr + ".json");
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		...
		async function checkForUpdates(isManual = false) {
            try {
                ...
                // Cache-busting: a unique query string on every single check so that
                ...
                if (latestVersion && compareRahinVersions(latestVersion, CURRENT_VERSION) > 0) {
                    ...
                    if (isManual) {
                        alert('You are using the latest version of RAHIN Panel.');
                    }
                }
            } catch (err) {
                if (isManual) {
        ...
        async function applyUpdate(token = null) {
            ...
            try {
                ...
                if (res.ok && data.success) {
                    ...
                    alert('خطا در بروزرسانی: ' + (data.error || 'نامشخص') + '\n\nدر صورت تکرار، از دکمه "آپدیت دستی" استفاده کنید.');
                    btn.disabled = false;
                }
            } catch (err) {
                alert('خطا در ارتباط با سرور: ' + (err && err.message ? err.message : 'نامشخص') + '\n\nاز گزینه آپدیت دستی استفاده کنید.');
                btn.disabled = false;
            }
        }
let cachedIpsData = {};
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/rahinvpn445-web/RAHIN-PANEL/main/ips.txt');
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('rahin_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پنل تغییر کرد');
            };

            const savedRate = localStorage.getItem('rahin_refresh_rate');
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پنل راهین - وضعیت اشتراک</h1>
