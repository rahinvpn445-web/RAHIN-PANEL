			...
			try {
				...
				const metadata = {
					...
					bindings: [
						...
						{ type: "kv_namespace", name: "KV", id: kvUuid },
				...
				const panelRes = await fetch(`https://raw.githubusercontent.com/rahinvpn445-web/RAHIN-PANEL/main/panel.html?cb=${Date.now()}`, { cache: "no-store" });
...
async function fetchPanelVersion(token, scriptName, latestVersion, devSub) {
    try {
        ...
        if (versionText && btnContainer) {
            ...
            let panelUrl = "#";
            if (devSub) {
                panelUrl = "https://" + scriptName + "." + devSub + ".workers.dev/panel369";
            }
